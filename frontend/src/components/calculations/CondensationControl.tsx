...
interface CondensationControlProps {
  results?: any;
}

export const CondensationControl = ({ results }: CondensationControlProps) => {
  const [dewPointTemp, setDewPointTemp] = useState(12.8333);
  const [relativeHumidity, setRelativeHumidity] = useState(49.2);
  const [wetBulbTemp, setWetBulbTemp] = useState(16.8);
    ...
    <div className="border rounded-md p-4 mb-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h3 className="font-medium">Condensation Control</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div>
          <label className="block text-sm text-gray-600 mb-1" htmlFor="dewPointTemp">Dew Point Temp</label>
          <div className="flex">
            <Input 
              id="dewPointTemp" 
              type="number" 
              value={dewPointTemp}
              onChange={(e) => setDewPointTemp(Number(e.target.value))}
              className="flex-1" 
            />
        ...
        <div>
          <label className="block text-sm text-gray-600 mb-1" htmlFor="relativeHumidity">Relative Humidity</label>
          <div className="flex">
            <Input 
              id="relativeHumidity" 
              type="number" 
        ...
        <div>
          <label className="block text-sm text-gray-600 mb-1" htmlFor="wetBulbTemp">Wet Bulb Temp</label>
          <div className="flex">
            <Input 
              id="wetBulbTemp" 
              type="number" 
              value={wetBulbTemp}
              onChange={(e) => setWetBulbTemp(Number(e.target.value))}
              className="flex-1" 
            />
            <div className="ml-2 px-2 py-1 bg-gray-100 rounded flex items-center">°C</div>
          </div>
